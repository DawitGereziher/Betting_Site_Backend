...
exports.getMarketCode = getMarketCode;
exports.getSelectionCode = getSelectionCode;
exports.normalizeMarkets = normalizeMarkets;
...
function getMarketCode(apiName) {
    ...
    if (lower.includes("both teams score") || lower.includes("both teams to score") || lower === "btts")
        return "BTTS";
    if (lower.includes("home team total") || lower === "total - home")
        return "HOME_TEAM_TOTAL";
    if (lower.includes("away team total") || lower === "total - away")
        return "AWAY_TEAM_TOTAL";
    if (lower.includes("goals over/under") || lower.includes("total goals"))
        return "TOTAL_GOALS";
    if (lower === "draw no bet")
        return "DRAW_NO_BET";
    if (lower.includes("asian handicap"))
        return "ASIAN_HANDICAP";
    if (lower.includes("handicap") && !lower.includes("asian"))
        return "EUROPEAN_HANDICAP";
    if (lower === "first half winner")
        return "FIRST_HALF_WINNER";
    if (lower.includes("first half") && (lower.includes("goals") || lower.includes("over/under")))
        return "FIRST_HALF_GOALS";
    if (lower.includes("second half") && (lower.includes("goals") || lower.includes("over/under")))
        return "SECOND_HALF_GOALS";
    if (lower === "second half winner")
        return "SECOND_HALF_WINNER";
    if (lower.includes("ht/ft") || lower.includes("half-time/full-time"))
    ...
    if (lower.includes("clean sheet"))
        return lower.includes("home") ? "CLEAN_SHEET_HOME" : "CLEAN_SHEET_AWAY";
    if (lower.includes("win to nil"))
        return lower.includes("home") ? "WIN_TO_NIL_HOME" : "WIN_TO_NIL_AWAY";
...
function getSelectionCode(value, // Allow loose types
marketCode, marketName = "" // Added marketName for fallback line extraction
) {
    ...
    if (marketCode === "MATCH_WINNER" || marketCode === "FIRST_HALF_WINNER" || marketCode === "SECOND_HALF_WINNER") {
        if (lower === "home")
            return { code: "HOME", line: null };
        if (lower === "draw")
            return { code: "DRAW", line: null };
        if (lower === "away")
            return { code: "AWAY", line: null };
    }
    // DOUBLE_CHANCE
    if (marketCode === "DOUBLE_CHANCE") {
        if (lower.includes("home") && lower.includes("draw"))
            return { code: "1X", line: null };
        if (lower.includes("home") && lower.includes("away"))
            return { code: "12", line: null };
        if (lower.includes("away") && lower.includes("draw"))
            return { code: "X2", line: null };
    }
    ...
    if (marketCode === "BTTS" || marketCode.includes("CLEAN_SHEET") || marketCode.includes("WIN_TO_NIL")) {
        if (lower === "yes")
            return { code: "YES", line: null };
        if (lower === "no")
            return { code: "NO", line: null };
    }
    // TOTAL_GOALS
    ...
    if (marketCode === "TOTAL_GOALS" || marketCode === "HOME_TEAM_TOTAL" || marketCode === "AWAY_TEAM_TOTAL" || marketCode.includes("FIRST_HALF_GOALS") || marketCode.includes("SECOND_HALF_GOALS")) {
        ...
        if (isOver)
            return { code: "OVER", line };
        if (isUnder)
            return { code: "UNDER", line };
    }
    ...
    if (marketCode === "ASIAN_HANDICAP" || marketCode === "EUROPEAN_HANDICAP") {
        ...
        if (isHome)
            return { code: "HOME", line };
        if (isAway)
            return { code: "AWAY", line };
        if (isDraw)
            return { code: "DRAW", line };
    }
    return { code: valStr, line: null };
}
function normalizeMarkets(apiResponse) {
    if (!(apiResponse === null || apiResponse === void 0 ? void 0 : apiResponse.response) || !Array.isArray(apiResponse.response) || apiResponse.response.length === 0) {
        return [];
    }
    // 1. FIXTURE LEVEL
